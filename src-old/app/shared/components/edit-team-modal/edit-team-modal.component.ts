...
export class EditTeamModalComponent implements OnInit {
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      this.message.success('團隊資訊已更新！');
      this.modal.destroy(true); // Return success to parent
    } catch (error) {
      console.error('[EditTeamModalComponent] ❌ Update team failed:', error);
      this.message.error('更新團隊失敗，請稍後再試');
